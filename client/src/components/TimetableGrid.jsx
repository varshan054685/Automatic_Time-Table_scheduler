      ...
      {isWebVisible && (
        ...
        >
          ...
          <div className="px-4 py-3 lg:p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center print:hidden">
            <div>
              <h3 className="text-base lg:text-xl font-bold text-slate-900">
                {entityData?.name}
              </h3>
              <p className="text-[10px] lg:text-sm text-slate-500 uppercase tracking-widest font-black">
                {isSectionView ? "Class Schedule" : "Faculty Load"}
              </p>
            </div>
          </div>
            ...
            >
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th
                    className="p-2 lg:p-4 text-left border-r border-slate-100 bg-slate-100/30 sticky left-0 z-10"
                    style={{ width: "80px" }}
                  >
                  ...
                  {uniqueSlots.map((slot, idx) => (
                    <th
                      key={idx}
                      className="p-1 lg:p-2 text-center border-r border-slate-100 last:border-0"
                    >
                      <div className="flex flex-col items-center gap-0.5 px-1 py-1 lg:px-2 lg:py-1.5 rounded-lg lg:rounded-xl bg-white shadow-sm border border-slate-100/50">
                        <span
                          className={`text-[7px] lg:text-[9px] font-black uppercase tracking-wider ${slot.label.toLowerCase().includes("break") ? "text-rose-500" : "text-indigo-600"}`}
                        >
                          {slot.label}
                        </span>
                  ...
                  >
                    ...
                    {uniqueSlots.map((slot, sIdx) => {
                      ...
                      return (
                        <td
                          key={sIdx}
                          className={`p-1 lg:p-2 border-r border-slate-100 last:border-0 relative transition-all ${isBreak ? "bg-slate-50/50" : ""}`}
                          style={{ height: "72px" }}
                        >
                          {isBreak ? (
                            <div className="flex flex-col items-center justify-center h-full opacity-20 group-hover:opacity-40 transition-opacity">
                              <div className="text-[7px] lg:text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">
                                {slot.label}
                              </div>
                            </div>
                          ) : entry ? (
                            ...
                            >
                              <div className="space-y-0.5 min-w-0">
                                <div className="flex items-center justify-between gap-0.5">
                                  <span
                                    className={`px-1 lg:px-1.5 py-px rounded text-[7px] lg:text-[8px] font-black uppercase tracking-tighter truncate ${entry.subject?.type === "lab" ? "bg-amber-100 text-amber-700" : "bg-indigo-100 text-indigo-700"}`}
                                  >
                              ...
                              <div className="pt-1 border-t border-slate-50 flex items-center justify-between gap-2 mt-auto">
                                ...
                                <div className="flex items-center gap-1 text-[7px] lg:text-[9px] font-bold text-slate-400 shrink-0">
                                  <MapPin className="w-2.5 h-2.5 lg:w-3 lg:h-3" />
                                  <span className="hidden sm:inline">
                                    {entry.classroom?.roomNumber || "R??"}
                                  </span>
